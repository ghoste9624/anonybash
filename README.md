...
NOTE: If you use this command without anonybash motd installed you will get an error in ``usr/etc/profile`` since you just removed the bottom line ``fi`` Run this command ``echo "fi" >> /data/data/com.termux/files/usr/etc/profile
termux-reload-settings 
clear && exit``
```
cd ~
